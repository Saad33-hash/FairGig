    ...
    <div className="bg-white rounded-2xl border border-slate-200 p-5 flex flex-col gap-3">
      <div className="flex items-start justify-between gap-3">
        <div className="flex flex-col gap-1">
          <div className="flex items-center gap-2 flex-wrap">
            <span className="text-xs font-bold text-slate-900">{g.platform}</span>
            <span className="text-slate-300">·</span>
            <span className="text-xs text-slate-500">{cat}</span>
            {worker?.city && <><span className="text-slate-300">·</span><span className="text-xs text-slate-500">{worker.city}</span></>}
          </div>
      ...
      {g.tags?.length > 0 && (
        <div className="flex flex-wrap gap-1.5">
          {g.tags.map((tag) => (
            <span key={tag} className="px-2.5 py-0.5 rounded-full bg-slate-100 text-slate-600 text-[11px] font-medium">{tag}</span>
          ))}
        </div>
      )}

      {g.clusterGroup && (
        <p className="text-xs text-blue-600 font-medium">Cluster: {g.clusterGroup}</p>
      )}
